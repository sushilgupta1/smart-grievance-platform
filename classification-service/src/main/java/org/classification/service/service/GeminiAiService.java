...
public class GeminiAiService {
	...
	public ClassificationResponse analyzeGrievance(String description)
	{
	    ...
	    try {
			...
			String cleanJson = rawJsonResponse.replace("```json", "").replace("```", "").trim();
			
			JsonNode resultNode=mapper.readTree(cleanJson);
			return new ClassificationResponse(resultNode.path("category").asText(),
					resultNode.path("priorityScore").asInt(1));
	    	
		} catch (Exception e) {
			System.err.println("CRITICAL: Gemini AI Engine Offline -"+e.getMessage());
			return new ClassificationResponse("Other",1);
		}
	}
}
